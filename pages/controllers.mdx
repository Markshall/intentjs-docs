...
For a class to be registered as a controller, it needs to be anotated with `@Controller` decorator from `@intentjs/common`. In this class, we will define our various routes.
...
```ts
import { HttpCode } from '@intentjs/common';
...
```ts
import { Header } from '@intentjs/common';
...
For Intent to know, you will need to register the `UserController` inside the `app/boot/sp/http.ts`. This enables our `HttpServiceProvider` to register the controller.
